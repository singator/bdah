...
author: '**Vik Gopal** (National University of Singapore) <br> **Wu Dan** (Khoo Teck
  Puat Hospital)'
date: "22 July 2014"
output:
...
A journey of a thousand miles begins with the **first** step. Some very basic inferential stats to get you started **comfortably** with R.
...
The following converts column names to upper case for ease of reference to the [FHS data documentation](https://biolincc.nhlbi.nih.gov/static/studies/teaching/framdoc.pdf). However, this conversion is generally neither necessary nor recommended. Instead, cultivate **good "R habits"** by learning from [Google's R Style Guide](https://google-styleguide.googlecode.com/svn/trunk/Rguide.xml). 
...
We wish to know whether the mean heart rate of "healthy" people at exam 1 was equal to 70 beats per minute (bpm). The criteria for "healthy" will be discussed soon.
...
We wish to know whether the mean heart rate of "healthy" people at exam 1 was equal to 70 bpm. [Null hypothesis](http://en.wikipedia.org/wiki/Null_hypothesis), $H_0: \mu = 70$. [Alternative hypothesis](http://en.wikipedia.org/wiki/Alternative_hypothesis), $H_1: \mu \neq 70$. We test the null hypothesis with a one-sample *t*-test at a [significance level](http://en.wikipedia.org/wiki/Statistical_significance) ($\alpha$) of 0.05.

```{r}
with(fhs.healthy, t.test(HEARTRTE, mu = 70))
```

**Interpretation**: Since $p > 0.05$, we fail to reject $H_0$ at $\alpha = 0.05$. Therefore there is no sufficient evidence to suggest that the mean heart rate of "healthy" people at exam 1 was equal to 70 bpm. 


## About Null Hypothsis ;) {.smaller}
...
2a. We wish to know whether there was a difference in mean heart rates at exams 1 and 2. 

2b. We wish to know whether there was a difference in mean heart rates between men and women at exam 1.
...
We wish to know whether there was a difference in mean heart rates at exams 1 and 2. $H_0: \mu_d = 0$. $H_1: \mu_d \neq 0$. We test the null hypothesis with a paired *t*-test at $\alpha = 0.05$.

```{r}
with(fhs.wide, t.test(HEARTRTE1, HEARTRTE2, paired = TRUE))
```

**Interpretation**: Since $p < 0.05$, we reject $H_0$ at $\alpha = 0.05$. Therefore there is strong evidence to suggest that there was a difference in mean heart rates at exams 1 and 2. 
...
We wish to know whether there was a difference in mean heart rates between men and women at exam 1.
...
We wish to know whether there was a difference in mean heart rates between men and women at exam 1. $H_0: \mu_1 - \mu_2 = 0$. $H_1: \mu_1 - \mu_2 \neq 0$. We test the null hypothesis with an independent two-sample *t*-test at $\alpha = 0.05$.
...
3a. We wish to know whether there was a difference in body mass index (BMI) among people with different education levels at exam 1. 

3b. If there were a difference, we wish to know which education levels differed from one another.
</div>
...
We wish to know whether there was a difference in mean body mass indices (BMIs) among people with different education levels at exam 1. $H_0: \mu_1 = \mu_2 = \mu_3 = \mu_4$. $H_1:$ otherwise. We test the null hypothesis with a one-way ANOVA (analysis of variance) *F*-test at $\alpha = 0.05$.

```{r}
summary(aov(BMI ~ as.factor(EDUC), data = filter(fhs, PERIOD == 1)))
```

**Interpretation**: Since $p < 0.05$, we reject $H_0$ at $\alpha = 0.05$. Therefore there is strong evidence to suggest that there was a difference in mean BMIs among people with different education levels at exam 1.
...
If there were a difference (in Task 2a), we wish to know which education levels differed from one another. The Tukey's HSD (honest significant difference) test examines all pairwise differences in means. 
