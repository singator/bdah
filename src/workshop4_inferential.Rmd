...
```{r message=FALSE}
fhs <- read.csv("../data/frmgham2.csv")
with(fhs, summary(HEARTRTE))

# library(Hmisc)
# with(fhs, describe(HEARTRTE)) # Hmisc::describe

library(psych)
with(fhs, describe(HEARTRTE)) # psych::describe

# library(foreign)
# fhs.yha <- read.dta("../data/tmp/healthyyoungadults.dta")
# with(fhs.yha, hist(heartrte2)) # to add ggplot2 version?
with(fhs, hist(HEARTRTE))
```
